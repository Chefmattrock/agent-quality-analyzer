#!/usr/bin/env python3
"""
Grant Program Builders Agent Analysis (Fixed Review Scoring)
...
4. Calculates reviews metrics ONLY for agents with 1+ reviews (excludes 0-review agents)
...
def get_user_agents_detailed(user_token, db_path=None):
    ...
    for row in results:
        ...
        # FIXED: Only set average_rating if agent has reviews, otherwise leave as None
        average_rating = reviews_score if reviews_count > 0 else None
        ...
        agent_data = {
            ...
            'average_rating': average_rating  # Can be None for agents with no reviews
        }
...
def calculate_weighted_average_rating(agents):
    """Calculate weighted average rating ONLY for agents with reviews."""
    agents_with_reviews = [a for a in agents if (a['reviews_count'] or 0) > 0]
    
    if not agents_with_reviews:
        return None, 0
    
    total_weighted_rating = sum(a['average_rating'] * a['reviews_count'] for a in agents_with_reviews)
    total_reviews = sum(a['reviews_count'] for a in agents_with_reviews)
    
    avg_rating = total_weighted_rating / total_reviews if total_reviews > 0 else None
    return avg_rating, len(agents_with_reviews)

def main():
    print("🔍 Analyzing Grant Program Builders and Their Agents (Fixed Review Scoring)")
    print("=" * 75)
    ...
    for i, contact in enumerate(contacts, 1):
        ...
        # FIXED: Calculate average rating only for agents with reviews
        contact_avg_rating, contact_reviewed_agents = calculate_weighted_average_rating(agents)
        ...
        contact_summary.append({
            ...
            'agents_with_reviews': contact_reviewed_agents,
            'avg_rating': contact_avg_rating  # Only for agents with reviews
        })
        
        review_info = f" (avg rating: {contact_avg_rating:.2f} from {contact_reviewed_agents} reviewed agents)" if contact_avg_rating else " (no reviewed agents)"
        print(f"   Found {len(agents)} agents after filtering ({total_before_filter} before, {private_low_exec} filtered out)")
        print(f"   ({len([a for a in agents if a['status'] == 'public'])} public, {len([a for a in agents if a['status'] == 'private'])} private){review_info}")
    ...
    print(f"\n{'='*75}")
    print("📊 GRANT PROGRAM BUILDERS ANALYSIS SUMMARY (FIXED)")
    print(f"{'='*75}")
    ...
    # FIXED: Calculate overall average rating only for agents with reviews
    overall_avg_rating, total_reviewed_agents = calculate_weighted_average_rating(all_agents)
    agents_without_reviews = len(all_agents) - total_reviewed_agents
    
    if overall_avg_rating:
        print(f"Overall average rating: {overall_avg_rating:.2f} (from {total_reviewed_agents} agents with reviews)")
        print(f"Agents without reviews: {agents_without_reviews} (excluded from rating calculation)")
    else:
        print(f"Overall average rating: N/A (no agents have reviews)")
    ...
    agents_output = os.path.join(output_dir, 'grant_program_builders_agents_fixed.csv')
    ...
    summary_output = os.path.join(output_dir, 'grant_program_builders_summary_fixed.csv')
    ...
    ids_output = os.path.join(output_dir, 'grant_program_builders_agent_ids_fixed.csv')
    ...
    # Print additional review statistics
    print(f"\n📊 REVIEW STATISTICS:")
    print(f"├── Total agents: {len(all_agents)}")
    print(f"├── Agents with reviews: {total_reviewed_agents}")
    print(f"├── Agents without reviews: {agents_without_reviews}")
    print(f"└── Review coverage: {(total_reviewed_agents/len(all_agents)*100):.1f}% of agents have reviews")
